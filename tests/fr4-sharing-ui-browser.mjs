...
try{
  ...
  stage='copy read-only link';await owner.page.locator('#copy-plan-link').click();const link=await owner.page.evaluate(()=>window.__fr4CopiedLink);const parsed=new URL(link);check('Copied link uses same-origin viewer-only fragment',parsed.origin===base&&parsed.hash===`#view-trip=${creation.id}.${creation.viewerToken}`);
