...
try{
 ...
 check('map tabs share one compact header row with the resize handle',await page.locator('.guidance-page-strip').evaluate(e=>{const r=e.getBoundingClientRect(),p=document.querySelector('#main').getBoundingClientRect(),h=document.querySelector('#journey-sheet-handle').getBoundingClientRect(),tabs=[...e.querySelectorAll('button')].map(b=>b.getBoundingClientRect());return e.closest('.journey-sheet-header')&&r.top>=p.top&&r.bottom<=p.top+60&&r.left>=h.right&&r.height<=60&&tabs[0].top===tabs[1].top&&tabs.every(t=>t.height>=44);})&&await page.locator('.map-page-swipe-hint').count()===0);
