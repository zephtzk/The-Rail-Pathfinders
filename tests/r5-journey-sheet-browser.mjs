...
try{
 ...
 check('map tabs dock to the panel in a compact single row',await page.locator('.guidance-page-strip').evaluate(e=>{const r=e.getBoundingClientRect(),p=document.querySelector('#main').getBoundingClientRect(),tabs=[...e.querySelectorAll('button')].map(b=>b.getBoundingClientRect());return e.closest('#journey-sheet-dock')&&r.top===p.top&&r.height<=60&&tabs[0].top===tabs[1].top&&tabs.every(t=>t.height>=44);})&&await page.locator('.map-page-swipe-hint').count()===0);
