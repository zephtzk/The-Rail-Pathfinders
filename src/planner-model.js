...
export function plannerFeedback(result, input = {}, {railCoverage = {}, busCoverage = {}} = {}) {
  ...
  if (result.status === 'unsupported-bus-window') return limitation('bus coverage', `Bus estimates are available on ordinary weekdays${busCoverage.validFrom && busCoverage.validThrough ? ` from ${busCoverage.validFrom} to ${busCoverage.validThrough}` : ''}, ${clock(busCoverage.earliestSeconds ?? 34200)}–${clock(busCoverage.latestSeconds ?? 59400)} SGT. Try a time in that window or choose rail stations. This is a prototype data limit, not a statement that buses are not running.`);
