  ...
  const plans=payload.itineraries.slice(0,3).flatMap((itinerary,option)=>{
    try{
      ...
      for(const leg of itinerary.legs){
        ...
        const source={type:walk?'walk':'ride',mode:walk?'walk':leg.mode==='BUS'?'bus':'rail',fromLabel,toLabel,startSeconds:(leg.startTime-midnight)/1000,endSeconds:(leg.endTime-midnight)/1000,durationSeconds:duration,timing:'provider-planned',...(leg.mode==='BUS'?{serviceNo:text(leg.route,80)}:{routeId:text(leg.route,80)})};
