...
export function staffCardForExecution(execution,request='automatic'){
...
export function createStaffSelection(){let contextKey=null,request='automatic';return {select(value){request=STAFF_REQUESTS.some(r=>r.id===value)?value:'automatic';},resolve(execution){const changed=contextKey!==null&&contextKey!==execution.contextKey;if(contextKey!==execution.contextKey)request='automatic';contextKey=execution.contextKey;return {...staffCardForExecution(execution,request),selectionReset:changed};}};}
...
export function mountStaffCard(host,{getActive=()=>null,getPrepared=()=>null,name,onBack=()=>{},onStationGuide=()=>{}}={}){
  ...
  function render(){const execution=resolveCurrentExecution(getActive(),{prepared:getPrepared(),name}),card=selection.resolve(execution);lastCard=card;
    ...
    host.innerHTML=`<div class="staff-page"><p class="field-note">Show this screen to a member of staff. Opening it does not contact anyone or share your trip.</p>${staffCardHTML(card)}<p class="staff-update-note" role="status">${card.selectionReset?'Your instruction changed. The card now follows your current step.':''}</p><label class="staff-request-label">Message to show<select data-staff-action="request">${STAFF_REQUESTS.map(r=>`<option value="${r.id}" ${r.id===card.request?'selected':''}>${esc(r.label)}</option>`).join('')}</select></label>${card.request!=='automatic'?'<button class="secondary" data-staff-action="automatic">Use my current instruction</button>':''}<div class="staff-page-actions"><button class="primary" data-staff-action="back">Back to current guidance</button><button class="secondary" data-staff-action="station">Station guide</button></div></div>`;
    host.querySelector('[data-staff-action="request"]').onchange=e=>{selection.select(e.target.value);render();};
