...
`:`${showHeader?`<p class="trip-ended-message">${active.status==='completed'?'You confirmed arrival. Location collection and further sharing have stopped on this device.':'This trip was cancelled. Location collection and further sharing have stopped on this device. It was not added to completed journeys or Journey spending.'}</p>`:''}${active.status==='completed'?'<button id="retry-fare" class="primary">View or record journey spending</button>':''}${sharing.session?.travellerToken?'<p class="muted">Your caregiver may still see the last received update until this device reconnects. Their access to the shared trip is unchanged.</p>':''}`}
  ...
  let planShareCopyMessage='Select the link to copy it.';
  function providerLink(){return sharing.session?.purpose==='plan-view'?sharing.links().viewer:null;}
  function renderPlanShare(){
    const owner=!!sharing.session?.editorToken,legacy=sharing.session?.purpose!=='plan-view',plan=legacy?null:shareView?.sharedPlan,link=providerLink();
    $('companion-sharing').innerHTML=`<h3>Shared trip</h3>${plan?`<h4>${esc(plan.origin?.label??plan.origin?.id)} → ${esc(plan.destination?.label??plan.destination?.id)}</h4>`:''}${owner&&legacy?'<p>Cancel this older share, then create a new read-only recipient link.</p>':''}${owner&&link?`<label for="plan-recipient-link">Recipient link</label><input id="plan-recipient-link" type="text" readonly spellcheck="false" autocomplete="off" value="${esc(link)}" aria-describedby="plan-share-copy-status"><p id="plan-share-copy-status" role="status">${esc(planShareCopyMessage)}</p>`:''}<div class="plan-share-actions"><button id="refresh-plan-link" class="primary">Refresh shared trip</button>${owner?'<button id="delete-plan-link">Cancel sharing</button>':''}</div>`;
    $('plan-recipient-link')?.addEventListener('focus',event=>event.target.select());
    $('refresh-plan-link').onclick=run(async()=>{
      const button=$('refresh-plan-link');button.disabled=true;
      try{if(owner&&!legacy){const plan=currentSharePlan();if(!plan||!validatePlan(plan))throw Error('Plan a trip before refreshing the shared link.');await sharing.read();await sharing.propose(plan);}await refreshShare();message(owner&&!legacy?'The shared link now shows your current planned trip.':'Shared trip refreshed.');}
      finally{if(button.isConnected)button.disabled=false;}
    });
    if(owner)$('delete-plan-link').onclick=run(async()=>{await sharing.delete();shareView=null;planShareCopyMessage='Select the link to copy it.';renderSharing();message('Sharing cancelled. The old recipient link no longer works.');});
  }
  function renderSharing(){if(sharing.session?.purpose==='plan-view'||sharing.session?.editorToken&&!sharing.session?.travellerToken){renderPlanShare();return;}const s=sharing.session,p=active?.permissions;const ended=!active||['completed','cancelled'].includes(active.status);$('companion-sharing').innerHTML=`<h3>Caregiver preparation &amp; sharing</h3>${!s&&(currentSharePlan())?'<button id="prepare-link" class="primary">Create recipient link</button>':''}<p>Share a read-only view of your current trip, including its addresses and route.</p>
    ${s?`<p>Role: ${esc(s.role)} · ${esc(shareView?.status??'awaiting refresh')} · server revision ${esc(s.revision)}.</p><button id="refresh-share">Refresh shared trip</button><button id="delete-share">Delete shared data</button>${s.editorToken?'<button id="propose-plan">Propose currently reviewed plan</button>':''}<div id="share-links"></div><div id="shared-view"></div>${s.role==='traveller'?`<div class="permissions"><p><strong>Separate permissions</strong> · ${s.accessRevoked?'ACCESS REMOVED ON SERVER':s.pendingPrivacyAction==='revoke'?'REVOCATION PENDING · collection stopped on this device':s.pendingPrivacyAction==='stop-location'?'LOCATION STOP PENDING ON SERVER · collection stopped on this device':p?.paused?'SHARING PAUSED':'sharing controlled below'}</p>${s.pendingPrivacyAction?'<button id="retry-privacy">Retry pending privacy change</button>':''}<label><input id="share-progress" type="checkbox" ${p?.progress?'checked':''} ${ended||p?.revoked?'disabled':''}> Share progress and accepted route changes</label><label><input id="share-location" type="checkbox" ${p?.location?'checked':''} ${ended||p?.revoked?'disabled':''}> Share geographic location (accuracy and timestamp)</label><button id="apply-sharing" ${ended||p?.revoked?'disabled':''}>Apply permissions</button><button id="pause-sharing" ${ended||p?.revoked?'disabled':''}>${p?.paused?'Resume':'Pause'} sharing</button><button id="revoke-sharing" ${s.accessRevoked?'disabled':''}>${s.pendingPrivacyAction==='revoke'?'Retry removing':'Remove'} caregiver access</button></div>`:''}`:'<p>No shared trip on this device. Review the selected journey, then create a recipient link.</p>'}
    `;
    $('prepare-link')?.addEventListener('click',run(async()=>{if(sharing.session)throw Error('Manage the existing shared trip before creating another link. Pending privacy changes must finish first.');const plan=currentSharePlan();if(!plan)throw Error('Choose and confirm a planned journey first.');await sharing.create(plan,{purpose:"plan-view"});shareView=await sharing.read();planShareCopyMessage='Select the link to copy it.';renderSharing();try{if(typeof navigator.clipboard?.writeText!=='function')throw Error();await navigator.clipboard.writeText(providerLink());planShareCopyMessage='Recipient link copied.';}catch{planShareCopyMessage='Select the link and use Copy to send it.';}if($('plan-share-copy-status'))$('plan-share-copy-status').textContent=planShareCopyMessage;message('Recipient link created.');}));
  ...
  // Notifications are off for this interface, including a subscription left by
  // an older build. Never request notification permission or subscribe here.
  Promise.resolve(navigator.serviceWorker?.getRegistration?.()).then(async registration=>{const subscription=await registration?.pushManager?.getSubscription?.();if(subscription)await subscription.unsubscribe();}).catch(()=>{});
