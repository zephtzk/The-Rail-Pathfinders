...
export function mountCompanion({getSelected=()=>null,getPlaces=()=>[],onSelectPlan=()=>{},onEndpoint=()=>{},name=id=>id,getMap=()=>null,getFareOptions=()=>({}),getRoutingContext=()=>null,getLocationContext=()=>({}),mode='real',host:mountHost=null,sectionHosts={},showHeader=true,showDock=true}={}){
  ...
  function renderSharing(){const s=sharing.session,p=active?.permissions;const ended=!active||['completed','cancelled'].includes(active.status);$('companion-sharing').innerHTML=`<h3>Caregiver preparation &amp; sharing</h3>${!s&&(prepared||active&&!ended)?'<button id="prepare-link" class="primary">Create recipient link</button>':''}<p>Create a link after reviewing a planned trip. Addresses included in that trip will be sent to the sharing service. Links expire; the recipient must explicitly accept.</p>
    ...
    `;
    $('prepare-link')?.addEventListener('click',run(async()=>{if(sharing.session)throw Error('Manage the existing shared trip before creating another link. Pending privacy changes must finish first.');const plan=prepared??(!ended?active?.plan:null);if(!plan)throw Error('Choose and confirm a planned journey first.');await sharing.create(plan);shareView=await sharing.read();renderSharing();message('Recipient link created. The recipient must review and accept; progress and location permissions start off.');}));
