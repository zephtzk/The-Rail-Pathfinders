...
export function mountEndpointSearch({field,box,status,clear,getSuggestions,isSelected,onEdit,onChoose,search=createOneMapAddressSearch()}){
  const key=field.id;
  let epoch=0,items=[],index=-1,busy=false;
  const announce=text=>{status.textContent=text;status.hidden=!text;};
  const cancel=()=>{epoch++;search.cancel();busy=false;field.removeAttribute('aria-busy');};
  function close(){
    if(busy)announce('');
  ...
  function render(points,{allowSearch=true}={}){
    ...
    if(allowSearch&&query.length>=2&&query.length<=160)items.push({query});
  ...
  function activate(i){const item=items[i];if(item?.point)select(item.point);else if(item?.query)void lookup();}
  field.oninput=()=>{reset();onEdit();showLocal();};
  field.onfocus=()=>{if(!isSelected()){reset();showLocal();}};
  field.onblur=()=>{close();};
