...
export function mountJourneySheet({shell,panel,dock,handle,content,mapPanel,nav,header,getView=()=> 'plan',onResize=()=>{}}){
  ...
  mobile.addEventListener('change',reset);window.addEventListener('resize',scheduleLayout);window.addEventListener('copilot:text-size-changed',scheduleLayout);window.addEventListener('blur',cancel);window.addEventListener('pointerdown',additionalPointer);
  layout();
  return {reset,revealPanel(){if(enabled){setState('expanded');panel.scrollTop=0;}},revealSurface(){if(enabled)setState('compact');},getState:()=>state,destroy(){clearGesture();observer.disconnect();cancelAnimationFrame(frame);mobile.removeEventListener('change',reset);window.removeEventListener('resize',scheduleLayout);window.removeEventListener('copilot:text-size-changed',scheduleLayout);window.removeEventListener('blur',cancel);window.removeEventListener('pointerdown',additionalPointer);for(const [name,listener] of Object.entries(listeners))handle.removeEventListener(name,listener);enabled=false;state='normal';render();delete panel.dataset.sheetState;shell.style.removeProperty('--journey-surface-top');}};
}
