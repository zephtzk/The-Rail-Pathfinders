...
- The existing R5 address browser fixture uses Bus 23A, whose fare category is unclassified in this snapshot. Keep its original unavailable-fare assertion. Supported OneMap services show `Planned fare: $… estimate` and `Approximate distance`, as covered by the focused FR2 tests. Accessibility stays `unknown` and source stays OneMap.
