...
FR4 follow-up: the provider interface now exposes only **Refresh shared trip** and **Cancel sharing** after link creation. The recipient link remains selectable and is copied once when the browser permits. Refresh publishes the current selected plan, while cancellation invalidates the link and leaves the provider's journey intact. Older invitation shares must be cancelled and recreated to get a read-only link. Journey notifications are disabled in this build; the Web Push design below is retained as historical technical documentation.
