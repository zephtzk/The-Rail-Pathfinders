...
The [desktop benchmark](evidence/r5/bus-benchmark.json) records 24 mixed/bus/rail queries, source hashes and Node/runtime flags. Final cold parse/index/first result: **180.3 ms**; warm p95 **162.6 ms**; combined gzip data **2,611,239 bytes**. The arrival index has 16,253 exact keys; 1,000 three-prediction matches took 3.82 ms after indexing. CPU/payload targets passed. Queries use the app's default 30-minute walking allowance.

The [allocation review](R5_PERFORMANCE.md) reduced the original benchmark's post-query sampled peak from 434.3 MiB to **143.3 MiB**, below the unchanged 150 MiB heap target. The final [inside-search probe](evidence/r5/bus-memory-probe.json) also passed: **149.75 MiB heap across 10,347 samples**, with external buffers and RSS separately reported. Neither workload forces collection during searches. Exact peaks between samples, physical-phone memory and performance remain unverified; the measured headroom is small, not a universal memory ceiling. The final benchmark retains 83.1 MiB after its ending collection, including bounded reusable private search storage. [Before-optimization evidence](evidence/r5/bus-benchmark-before-memory.json) is retained separately.

The [browser worker checks](evidence/r5/bus-browser.json) use actual clicks/keyboard on desktop Edge with mobile/touch emulation: editing the destination cancels the in-flight search, stale results stay absent, a new search succeeds, accepted journeys remain unchanged, and frames continue. Eight checks passed with no browser errors; the evidence records that run's frame and query timings. These observations are separate from Node heap measurement and do not establish physical-phone responsiveness.
