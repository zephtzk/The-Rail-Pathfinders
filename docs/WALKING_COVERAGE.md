...
## Phase 4 acquisition metadata correction

`data/bus/walking-evidence/acquisition.json` pins the four source hashes and explicit acquisition uncertainty. No independent original acquisition clock was retained, so `retrievedAt` is null and `retrievalTimeStatus` is unknown. Former mtime-derived values remain as `legacyReportedRetrievedAt`; they are not verified acquisition timestamps. `archivedBy` identifies the first source-containing commit time, not download time. Supported SMRT sourceDate and OSM object edit timestamps are preserved. Ledger version is 2026-09-18.2; geometry and field/accessibility limitations are unchanged. The real generator is tested after copying files and setting mtimes to 2000/2040; outputs remain byte-identical. Source-byte changes fail the pinned metadata gate. Earlier wording about retrieval time describes the superseded Phase 3 ledger.
